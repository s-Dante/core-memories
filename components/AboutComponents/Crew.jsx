// components/AboutComponents/Crew.jsx

import { Gafete } from "@/components";
import { getCrew } from "@/app/lib";

export default async function Crew() {
    const data = await getCrew();
    const crewList = Array.isArray(data) ? data : data.crew;

    return (
        <section aria-labelledby="crew-heading">
            {/* --- INICIO DE CAMBIOS: Reducimos el margen superior --- */}
            <h2 id="crew-heading" className="text-4xl md:text-5xl font-extrabold uppercase tracking-tight text-white mb-8 text-center sm:text-left">Crew</h2>
            {/* --- FIN DE CAMBIOS --- */}
            <div className="grid gap-6 sm:gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
                {crewList.map((crew) => (
                    <Gafete key={crew.id} crew={crew} />
                ))}
            </div>
        </section>
    );
}